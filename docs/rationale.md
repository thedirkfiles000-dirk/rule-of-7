...
## The Error-Correction Frame

The Shannon and thermodynamic frames explain why a tight, non-redundant Background resists drift. Neither fully explains why the format includes an optional mechanism — the Reinforcer — whose entire function is deliberate redundancy. A third frame closes that gap, and it is not a metaphor: a well-placed Reinforcer is functionally a repetition code, the simplest error-correcting code known.

**What error correction actually does.** A message travels through a channel that can corrupt it. The sender doesn't prevent corruption — the channel is noisy by nature — but adds structured redundancy so the receiver can recover the original message despite it. The simplest form, a repetition code, sends the same bit multiple times; the receiver takes a majority vote. It is inefficient in Shannon's sense — most of the transmitted bits carry no new information — but it is not wasteful. It is insurance, purchased with bits, against a channel known to drop information.

**The mapping is direct, not figurative.** The message is a load-bearing character fact. The channel is the context window as attention operates across it, and it is genuinely noisy: recency bias, competing narrative content, the same context-pressure degradation the thermodynamic frame already establishes as real and measurable in testing. The receiver is the model's next-token prediction at generation time, deep into a long conversation, drawing on whatever survived the channel intact. A Reinforcer — the same fact, restated once more, in different words, near the end of the document — is not summarizing. It is transmitting the same bit twice so that if the first transmission degrades under context pressure, the second is there to be recovered.

**This does not contradict the Shannon-efficiency principle; it operates on a different axis, the same way Shannon entropy and thermodynamic entropy operate on different axes without contradicting each other elsewhere in this document.** Shannon efficiency argues against *accidental* redundancy — three adjectival ways of saying "she's guarded," each spent for no additional constraint on generation. Error correction argues for *deliberate* redundancy, spent once, only on facts specific enough that losing them would break the scenario rather than merely soften it. The specification already gates Reinforcers this way — used only when a character carries facts so plot-critical that drift on a low-memory model would break the scenario — without naming the mechanism. The mechanism is error correction. The existing gating criterion is the correct one for deciding when a repetition code is worth its cost: not every fact deserves insurance, only the ones whose loss is catastrophic rather than cosmetic.

**Where the analogy holds precisely, and where it doesn't.** It holds precisely at the level of a repetition code: redundant transmission of a single bit of information to survive a lossy channel, at a measurable, honest cost in symbols spent. It does not extend to formal error-correcting codes with parity checks or syndrome decoding — there is no mechanism by which the model detects that a fact was corrupted and reconstructs it from a checksum. The model simply has two chances to encounter the fact instead of one, and the second, more recent chance is weighted more favorably by the same recency dynamics that cause drift in the first place. That is a weaker guarantee than a formal error-correcting code provides, but it is the same *kind* of guarantee, not a different one borrowing its name.

**A second strategy operates on the noise instead of the message: eliminate the channel.** Redundancy protects a fact traveling through a noisy channel. A stronger strategy is available for the highest-risk beats: remove the channel noise entirely by delivering the fact as fixed text rather than model-generated text. A Greeting is not generated in response to anything — it is transmitted verbatim, every session, with no attention-based degradation possible because there is no generation step to degrade. Testing has surfaced cases where a specific narrative beat proved resistant to correction even through an explicit negative-space Rule and a named, positively-identified alternative fact — a repeated failure despite two layers of redundancy. The fix that worked was not a third layer of redundancy. It was moving the vulnerable beat into the Greeting, scripted as fixed text, removing the generation step where the corruption was occurring. Error correction and channel elimination are both legitimate responses to noise; the choice between them is a judgment about whether a fact merely needs insurance or needs to never touch the noisy channel at all.
